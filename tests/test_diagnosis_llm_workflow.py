...
class DiagnosisLlmWorkflowTest(unittest.TestCase):
    ...
    def test_diagnosis_agent_allows_xray_normal_observation_with_mri_next_step(self):
        model_client = RecordingModelClient(
            response=ChatResponse(
                content=json.dumps(
                    {
                        "诊断倾向": "LLM 认为当前 X 光证据不足，需进一步评估",
                        "影像依据": ["当前 X 光未见明显异常"],
                        "分期判断": "暂无法可靠分期，建议 MRI 进一步排查早期股骨头坏死",
                        "不确定性说明": [
                            "普通 X 光对早期股骨头坏死敏感性有限，不能据此判断无病"
                        ],
        ...
        report = agent.generate_report(
            case_id="case_xray_normal_observation_needs_mri",
            patient_info={"symptoms": ["髋关节疼痛"]},
            visual_result=self._visual_result(),
            alignment_plan=alignment_plan,
        )
        ...
        self.assertIn("当前 X 光未见明显异常", report["影像依据"])
        self.assertIn("不能将 X 光未见异常解释为无病", report["不确定性说明"])
        self.assertTrue(any("MRI" in item for item in report["建议进一步检查"]))

    def test_diagnosis_agent_allows_limited_xray_negative_observation(self):
        model_client = RecordingModelClient(
            response=ChatResponse(
                content=json.dumps(
                    {
                        "诊断倾向": "LLM 认为当前 X 光证据不足，需进一步评估",
                        "影像依据": [
                            "早期股骨头坏死可能 X 光阴性，需要 MRI 进一步评估",
                            "囊性变未见标注，但不能替代完整影像诊断",
                        ],
                        "分期判断": "暂无法可靠分期，建议 MRI 进一步评估",
                        "不确定性说明": [
                            "当前 X 光不能作为排查早期股骨头坏死的充分证据"
                        ],
                        "建议进一步检查": ["双髋 MRI T1/T2/STIR"],
                        "治疗建议": ["骨科或影像科复核"],
                    },
        ...
        report = agent.generate_report(
            case_id="case_xray_negative_observation_needs_mri",
            patient_info={"symptoms": ["髋关节疼痛"]},
            visual_result=self._visual_result(),
            alignment_plan=alignment_plan,
        )
        ...
        self.assertNotIn("llm_fallback_reason", report)
        self.assertTrue(any("X 光阴性" in item for item in report["影像依据"]))
        self.assertTrue(any("MRI" in item for item in report["建议进一步检查"]))

    def test_diagnosis_agent_allows_zero_measurement_when_explicitly_limited(self):
        model_client = RecordingModelClient(
            response=ChatResponse(
                content=json.dumps(
                    {
                        "诊断倾向": "LLM 认为当前 X 光证据不足，需进一步评估",
                        "影像依据": [
                            "collapse_ratio 记录为0.0，但不能将该数值解释为塌陷程度判断的完整依据"
                        ],
                        "分期判断": "暂无法可靠分期，建议 MRI 进一步评估",
                        "不确定性说明": [
                            "股骨头形态及关节面轮廓评估信息不足，不能作为阴性结论"
                        ],
                        "建议进一步检查": ["双髋 MRI T1/T2/STIR"],
                        "治疗建议": ["骨科或影像科复核"],
                    },
        ...
        alignment_plan = self._alignment_plan(
            analysis_status="partial_evidence",
            blocked=["不能把 missing_input 当作阴性发现"],
        )
        agent = DiagnosisDoctorAgent(
            prompt_runner=PromptRunner(model_client=model_client)
        )
        ...
        report = agent.generate_report(
            case_id="case_zero_measurement_limited",
            patient_info={"symptoms": ["髋关节疼痛"]},
            visual_result=self._visual_result(),
            alignment_plan=alignment_plan,
        )
        ...
        self.assertNotIn("llm_fallback_reason", report)
        self.assertTrue(any("为0.0" in item for item in report["影像依据"]))

    def test_diagnosis_agent_does_not_treat_nonzero_decimal_as_zero(self):
        model_client = RecordingModelClient(
            response=ChatResponse(
                content=json.dumps(
                    {
                        "诊断倾向": "LLM 认为当前 X 光存在候选征象，需进一步评估",
                        "影像依据": ["lesion_area_ratio 为0.035，可作为候选范围参考"],
                        "分期判断": "暂无法可靠分期，建议 MRI 进一步评估",
                        "不确定性说明": [
                            "当前 X 光证据不足以完成早期股骨头坏死评估"
                        ],
                        "建议进一步检查": ["双髋 MRI T1/T2/STIR"],
                        "治疗建议": ["骨科或影像科复核"],
                    },
        ...
        self.assertNotIn("llm_fallback_reason", report)
        self.assertTrue(any("为0.035" in item for item in report["影像依据"]))
