...
class GuidelineSkillBuilderTest(unittest.TestCase):
    ...
    def test_guideline_search_finds_ipf_hrct_source(self):
        ...
        skill = SkillBuilderTool().build_guideline_skill_from_search(result)
...
"""
    ...
    def test_guideline_extraction_parses_documents_into_skill_payload(self):
        documents = [
            {
    ...
    def test_skill_builder_uses_guideline_search_for_guideline_based_skill(self):
        with TemporaryDirectory() as tmpdir:
            skill = SkillBuilderTool(skills_dir=Path(tmpdir)).prepare_skill(
                disease_key="femoral_head_necrosis",
                disease_name="股骨头坏死",
                observations=["髋关节疼痛"],
            )
        ...
        self.assertEqual(skill["evidence_level"], "high")
        self.assertEqual(skill["source_type"], "medical_guideline")
        self.assertIn("source_documents", skill)
        self.assertTrue(VisualProtocolValidator().validate_skill(skill)["valid"])
        ...
        self.assertIn("alignment_tasks", skill["visual_protocol"])
        self.assertEqual(skill["visual_protocol"]["required_next_images"][0]["modality"], "MRI")
        self.assertIn("blocked", skill["visual_protocol"]["diagnosis_scope"])

    def test_guideline_search_result_builds_actionable_guideline_skill(self):
        with TemporaryDirectory() as tmpdir:
            skill = SkillBuilderTool(skills_dir=Path(tmpdir)).prepare_skill(
                disease_key="diffuse_glioma_brats",
                disease_name="成人弥漫性胶质瘤",
        ...
        self.assertEqual(
            skill["guideline_source"]["source_catalog_path"],
            "data/guidelines/guideline_sources.json",
        )
        self.assertIn("guideline_extraction", skill)
        self.assertEqual(skill["guideline_extraction"]["tool"], "GuidelineExtractionTool")
        self.assertIn("citations", skill["guideline_extraction"])
        self.assertTrue(skill["guideline_extraction"]["citations"])
        self.assertIn("头痛", skill["clinical_features"]["common_symptoms"])
        self.assertIn("MRI FLAIR", skill["required_image_views"])
        self.assertIn("integrated_diagnosis_required", skill["staging_rules"])
        self.assertIn("whole tumor", skill["vision_agent_tasks"]["segmentation_targets"])
        self.assertEqual(skill["visual_protocol"]["disease_target"], "diffuse_glioma_adult")
        self.assertEqual(
        ...
        self.assertTrue(skill["visual_protocol"]["alignment_tasks"])
        self.assertTrue(skill["visual_protocol"]["required_next_images"])
        self.assertTrue(skill["visual_protocol"]["diagnosis_scope"]["blocked"])

    def test_skill_builder_marks_multi_source_conflicts_and_source_priority(self):
        guideline_result = {
        ...
        self.assertEqual(skill["source_priority"][0]["source_id"], "newer_guideline")
        self.assertEqual(skill["source_priority"][0]["publication_year"], "2024")
        self.assertEqual(skill["source_priority"][0]["region"], "global")
        self.assertEqual(skill["guideline_conflicts"][0]["field"], "required_image_views")
        self.assertEqual(skill["guideline_conflicts"][0]["resolution"], "merged_union_review_required")
        self.assertEqual(
            [source["source_id"] for source in skill["guideline_conflicts"][0]["sources"]],
            ["newer_guideline", "older_guideline"],
        )
        self.assertEqual(skill["quality_control"]["conflict_status"], "needs_review")
        self.assertEqual(skill["quality_control"]["conflict_count"], 1)

    def test_skill_builder_grades_conflicts_across_core_guideline_fields(self):
        guideline_result = {
        ...
        conflicts_by_field = {
            ...
            for conflict in skill["guideline_conflicts"]
        }
        ...
        self.assertEqual(skill["quality_control"]["highest_conflict_severity"], "high")
        self.assertEqual(skill["quality_control"]["conflict_severity_counts"]["low"], 2)
        self.assertEqual(skill["quality_control"]["conflict_severity_counts"]["medium"], 1)
        self.assertEqual(skill["quality_control"]["conflict_severity_counts"]["high"], 1)
        self.assertEqual(skill["quality_control"]["missing_core_sections"], [])
        self.assertEqual(skill["quality_control"]["core_section_status"], "complete")
    ...
    def test_skill_builder_quality_gate_marks_missing_core_sections(self):
        guideline_result = {
        ...
        self.assertIn("required_image_views", skill["quality_control"]["missing_core_sections"])
        self.assertIn("vision_agent_tasks", skill["quality_control"]["missing_core_sections"])
        self.assertEqual(skill["quality_control"]["core_section_status"], "incomplete")
        self.assertEqual(skill["quality_control"]["source_priority_status"], "implicit_order")
        self.assertEqual(skill["quality_control"]["formal_skill_status"], "needs_review")
        self.assertFalse(skill["quality_control"]["can_enter_formal_guideline_skill"])

    def test_skill_builder_uses_evidence_summary_for_missing_guideline(self):
        with TemporaryDirectory() as tmpdir:
            skill = SkillBuilderTool(skills_dir=Path(tmpdir)).prepare_skill(
        ...
        self.assertEqual(skill["source_type"], "internal_dataset_summary")
        self.assertEqual(skill["evidence_summary_mode"], "evidence_summary_mode")
        self.assertIn("不等同于正式医学指南", skill["warning"])
        self.assertEqual(skill["path_type"], "privileged_knowledge_discovery")
        self.assertEqual(skill["safety_gate"]["mode_required"], "hypothesis_validation")
        self.assertIn("确诊", skill["safety_gate"]["forbidden_claims"])
        self.assertEqual(
            skill["safety_gate"]["allowed_outputs"],
            ["early_risk_alert", "research_warning", "recommend_gold_standard_confirmation"],
        )
        self.assertEqual(skill["discovery_metadata"]["teacher_signal"], "not_configured_yet")
        self.assertEqual(skill["required_modalities"]["deployment"], ["low_cost_or_routine_image"])

    def test_skill_builder_can_persist_generated_skill_when_explicitly_requested(self):
        with TemporaryDirectory() as tmpdir:
            skills_dir = Path(tmpdir)
            skill = SkillBuilderTool(skills_dir=skills_dir).prepare_skill(
                disease_key="rare_disease_without_guideline",
                disease_name="罕见病示例",
                observations=["已确诊病例中常见局部纹理异常"],
                persist=True,
            ...
            saved_path = skills_dir / "rare_disease_without_guideline.yaml"
            self.assertTrue(saved_path.exists())
            self.assertIn(skill["skill_type"], saved_path.read_text(encoding="utf-8"))
