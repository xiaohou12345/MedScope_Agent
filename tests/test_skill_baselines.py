...
class SkillBaselineTest(unittest.TestCase):
    def test_fhn_finding_list_baseline_is_preserved_for_protocol_comparison(self):
        ...
        current_path = Path("skills/femoral_head_necrosis.yaml")

        baseline = json.loads(baseline_path.read_text(encoding="utf-8"))
        current = json.loads(current_path.read_text(encoding="utf-8"))

        self.assertEqual(baseline["skill_id"], "femoral_head_necrosis_v0.1")
