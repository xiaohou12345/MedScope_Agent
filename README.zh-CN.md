...
- `benchmarks/segmentation/` 已经把专病分割验证入口和 web demo 分离，并能对 metric-ready case 输出质量门通过/失败统计。
- Skill 自动升级被正确阻断，但后续如果要做 self-evolving，需要严格保留人工审核和 validation gate。
- 当前系统是科研 demo，不是临床验证系统。

建议下一步：

1. 向 `benchmarks/segmentation/` 增加真实标注 benchmark case，并在 manifest 中配置 `metric_gates`，让 Dice/IoU 不只停留在 readiness 检查。
