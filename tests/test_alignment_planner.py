...
class AlignmentPlannerTest(unittest.TestCase):
    ...
    def test_planner_uses_glioma_visual_protocol_for_partial_flair_mri(self):
        knowledge = json.loads(Path("knowledge/diffuse_glioma_brats.yaml").read_text(encoding="utf-8"))
        ...
        plan = AlignmentPlanner().build_plan(
            ...
            routing_decision={"selected_knowledge": "diffuse_glioma_brats"},
            disease_knowledge=knowledge,
        )
    ...
    def test_planner_handles_unmatched_knowledge_without_disease_specific_code(self):
        plan = AlignmentPlanner().build_plan(
            payload={
            ...
            routing_decision={"selected_knowledge": None},
            disease_knowledge={},
        )

        self.assertEqual(plan["analysis_status"], "partial_evidence")
        self.assertIsNone(plan["selected_knowledge"])
