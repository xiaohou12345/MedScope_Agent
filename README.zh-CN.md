...
- 管理 knowledge 和 visual protocol。
- 管理上传图片、mask、overlay、报告和 audit artifact。
- 根据 knowledge 和图像模态选择视觉工具。
...
- [docs/PROJECT_PROGRESS_SYNC_20260607.zh-CN.md](docs/PROJECT_PROGRESS_SYNC_20260607.zh-CN.md)
...
knowledge/       疾病 knowledge、指南来源、visual protocol
tests/        单元测试和集成测试
...
- `image_prompt_knowledge_baseline.json`：三层原始输出和指标。
- `image_prompt_knowledge_baseline.md`：三层对比表。
...
artifact。它用于测试上传、路由和 FHN knowledge 主线，不是临床图像，也不是分割 benchmark。
它不证明病灶检测质量。
...
- `guideline_based` 和 `data_mined_hypothesis` 的边界已经写进 knowledge contract。
- Memory 不只是保存结论，而是保存 patient/image/knowledge/reasoning 四类证据链。
...
- Knowledge 自动升级被正确阻断，但后续如果要做 self-evolving，需要严格保留人工审核和 validation gate。
- benchmark 结果不会更新临床诊断或正式 knowledge，只报告验证指标和质量门状态。
...
6. 保持 clinical knowledge 更新必须经过人工审核和验证门。
