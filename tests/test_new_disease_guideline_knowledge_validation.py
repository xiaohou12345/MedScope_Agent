...
from scripts.new_disease_guideline_knowledge_validation import (
    run_new_disease_guideline_knowledge_validation,
)


class NewDiseaseGuidelineKnowledgeValidationTest(unittest.TestCase):
    def test_ipf_guideline_knowledge_visual_evidence_bundle_and_safety_boundary(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            output_dir = Path(tmpdir) / "validation"

            payload = run_new_disease_guideline_knowledge_validation(output_dir=output_dir)
        ...
        self.assertEqual(summary["schema_version"], "new_disease_guideline_knowledge_validation.v1")
        self.assertEqual(summary["disease_key"], "idiopathic_pulmonary_fibrosis_hrct")
        self.assertEqual(summary["status"], "ok")
        self.assertTrue(summary["guideline_knowledge"]["generated"])
        self.assertEqual(summary["guideline_knowledge"]["knowledge_type"], "guideline_based")
        self.assertGreaterEqual(summary["guideline_knowledge"]["source_count"], 2)
        self.assertEqual(summary["guideline_knowledge"]["visual_protocol_status"], "valid")
        self.assertIn("HRCT chest", summary["guideline_knowledge"]["required_image_views"])
        ...
        self.assertFalse(summary["safety_boundary"]["formal_knowledge_updated"])
        ...
        self.assertIn("新病种 guideline knowledge 端到端验证", markdown)
